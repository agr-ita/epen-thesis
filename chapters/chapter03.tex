...
We tested the decoding performance on the validation set of AMs with total number of Gaussians going from 1000 to 15000, numbers which correspond to the default number of Gaussians used for training, respectively, non-speaker adapted monophone and triphone models in Kaldi. 
...
\begin{figure}[htb!]
  \centering
  \begin{overpic}[trim={0 1cm 0 1.5cm},clip, width=0.8\linewidth]{chapter03/m-ahpa_praat_ah0apa.pdf}\end{overpic}
...
%%% Correlation (only one value -> r) - Global
%%% Fine grain

\subsubsection{Results: All models}

\paragraph{All items: Overall ressemblance}
Blablabla {\color{red}Describe graph showing response patterns and then correlation analyses}

\paragraph{Filler items: Identification accuracy}
%%%Full vowel (control)
%% null not good enough (cf coart=E and coart=U) 
Blablabla
\subsubsection{Results: Focus on best model}
...
\paragraph{Identification task simulation}
We used the same procedure as in Experiment 1. An example of how the ASR system decodes the experimental stimuli can be seen in Figure \ref{fig:m-ahpa_align}. 
...
As a global measure of ressemblance 
...
{\color{red}Describe graph showing response patterns and then correlation analyses}

\paragraph{Overall ressemblance}
...
\paragraph{Coarticulation effects}
