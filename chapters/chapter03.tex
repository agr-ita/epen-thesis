...
An obvious next step would be to test the language models used in \cite{wilson2014}, where the authors found that most LMs performed better than the null LM at predicting data from a production task. In that work, the favoured LMs were the ones where phonotactic legality was a gradient process, where phones were represented with featural descriptions (e.g. {\color{red}[CITE Albright 2009]}) or where weights given to grammatical constraints were derived from the principle of maximum entropy (e.g. {\color{red}[CITE BoersmaPater2007; HayesWilson2008]}). In parallel, we can also explore whether our ASR system's acoustic model (accompanied by a null language model) is able to explain effects attributed to abstract grammatical processes. We will experiment this in the next section. 
%% Due to task?
...
%%%%%%%%%%%%
\newpage
\section{{\color{red}Medley of epenthetic variations: Phonological processes or embedded in the phonetics?}} \label{3-acmatch}
...
However, the acquisition of native phonetic categories has been described as a process of partitioning the phonetic space in an optimal way, respective to the phonemic needs of the native language \cite{kuhl1995, best1995}. Phonemic misperceptions can then result from mapping the nonnative input to a native category that is not congruent with the original nonnative, or that lacks finer grained separations resulting in additional phonemic contrasts.    
...
%  Rose
Within a given language, most episodes of epenthesis involve a ``default'' epenthetic vowel (e.g., \textipa{[W]} in Japanese, \textipa{[1]} in Korean, \textipa{[@]} in English). Aside from variations in epenthetic vowel quality due to coarticulation \cite{dupoux2011, guekozIS17, guekozJASA17}, variations have also been triggered by neighbouring consonants \cite{mattingley2015, durvasula2015, durvasula2018}.
...
\begin{itemize}
\item For Japanese listeners tested by \cite{mattingley2015}, the voiced alveolo-palatal affricate \textipa{/\t{d\textctz}/} triggered \textipa{[i]}-epenthesis and default \textipa{[W]}-epenthesis $92\%$ and $6\%$ of the time, respectively.
\item For Mandarin listeners tested
  by \cite{durvasula2018}, the aspirated alveo-palatal affricate \textipa{/\t{tC}\super h/} ellicited more \textipa{[i]}-epenthesis than other vowels (including the ``default'' \textipa{[@]}).
...
{\color{red}[TODO on Saturday]: Add paragraph about Durvasula2015 and phonological alternations in Korean.}

Given how we found front vowel coarticulation to be particularly salient at triggering \textipa{/i/}-epenthesis in both humans and our ASR models (cf. Figure \ref{fig:m-ahpa_test_heat}), we could hypothesize that similar acoustic cues may be found in palatal consonants. Can our AM-only ASR model reproduce the palatal effects observed by \cite{durvasula2015} without explicit knowledge about phonological alternations? We will study this in Experiment 1.   

\subsubsection{Variations in epenthesis due to syllabic structure}
% Church1987: "the author argues that allophony is not necessarily detrimental for speech recognition. Instead, subtle phonetic cues can help us find syllable boundaries (e.g. aspiration only occurs syllable-initially). "
% Monahan, too
% Berent is probably revelant as well
There has been evidence of syllabic structure being taken into consideration by the perceptual system when listening to nonnative speech. For instance, \cite{kabak2007} tested ... {\color{red}[TODO on Saturday]: Quickly describe Kabak\&Idsardi2007 experiment and results}. 
...
We could then hypothesize that the acoustic model in the perceptual system is aware of certain allophonic variations, instead of just having a single model per phoneme. In this case, phonotactics have an influence on the acoustic model while it is established, helping cluster phones into positionally-relevant groups, for instance. During perception, acoustic match may be considered between the acoustic input and the independent allophones of all phonemes.
...
\newpage
\subsection{{\color{red}Experiment 2: Variations due to syllabic structure}}
