...
%%%%%%%%%%%%
\newpage
\section{{\color{red}Medley of epenthetic variations: \\ Due to phonological processes or embedded in the phonetics?}} \label{3-acmatch}
...
\item For Mandarin listeners tested by \cite{durvasula2018}, the aspirated alveolo-palatal affricate \textipa{/\t{tC}\super h/} ellicited more \textipa{[i]}-epenthesis than other vowels (including the ``default'' \textipa{[@]}).
...
% Monahan, too? Berent is probably revelant as well?
...
An interesting remark is that, for many clusters that are phonotactically illegal in onset position, English listeners reported low rates of epenthesis in word-initial position, comparable to those of legal clusters. When ordering word-initial clusters by increasing rates of epenthesis, we saw that this order was grossly mimicked by the acoustic models. This is in line with the hypothesis advanced by \cite{wilson2014}, stating the influence of phonetic properties on cluster misproductions. Whether participants in our task were able to perceive word-initially illegal clusters as accurately as rates of epenthesis may suggest requires further investigation. Indeed, vowel epenthesis may have been blocked by other misperception processes such as deletion/cluster simplification (e.g., \textipa{/tkapa/} $\rightarrow$ \textipa{/kapa/}) or perceptual adaptation (e.g., \textipa{/tl/} $\rightarrow$ \textipa{/kl/}, as already attested by \cite{halle2007}).  
...
% AM probably needs LM (cf wpd effects), but not an n-gram one.
% The level of depth is unknown (maybe palatal effect will surface with acoustics x surface-level LM).   
...
%% Need same standards to allow for reliable cross-linguistic comparisons
