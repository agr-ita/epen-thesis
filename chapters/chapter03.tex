...
Speech audio waveforms are transformed into sequences of acoustic vectors $A_{1:T} = a_{1}, ..., a_{T}$ through the first step, called feature extraction. In the decoding phase that follows, the ASR system attempts to find the sequence of words $w_{1:L} = w_{1}, ..., w_{L}$ which is most likely to have produced the sequence of acoustic vectors $A$.
...
Put into Bayesian terms, this represents computing the posterior probability $P(w|A)$ of all combinations of words, given the acoustics. From this, the decoder then selects the sequence of words $w$ with highest posterior probability given the acoustic evidence $A$. However, due to the recursive and compositional nature of language, it is difficult to evaluate all possible combinations of words, in order to find the most probable one. As such, following Bayes's theorem, equation \ref{eq_hmm_dec_post} can be rearranged as:
...
The likelihood $P(A|w)$, given by the acoustic model, is the probability of the acoustics given the sequence of words. This is The prior $P(w)$, given by the language model, corresponds to the probability of the word sequence, and can be derived from frequency counts.
...
\begin{itemize}
\item What are MFCCs
  \item deltas + ddeltas (over 5 frames)
\item CMVN
\item Pitch
\item ...
\end{itemize}
...
% c1    0.4374 0.0003149    1.184     2876

%  R-structure:  ~units

%       post.mean l-95% CI u-95% CI eff.samp
% units   0.03142  0.02619  0.03674     4058

%  Location effects: modper.dist ~ lm 

%                  post.mean   l-95% CI   u-95% CI eff.samp   pMCMC   
% (Intercept)      0.4809114 -0.0640950  0.9562979     3556 0.05867 . 
% lmunigram        0.0253643 -0.0413540  0.0951623     3445 0.45813   
% lmbigram_online  0.0834542  0.0183947  0.1503082     3750 0.01387 * 
% lmbigram_retro  -0.0005802 -0.0619566  0.0685334     3750 0.99680   
% lmbigram_batch   0.0894134  0.0180592  0.1533017     3764 0.00907 **
% ---
% Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
% > posterior.mode(MC2.1Sol)
%     (Intercept)       lmunigram lmbigram_online  lmbigram_retro  lmbigram_batch 
%     0.473257621     0.033990801     0.079494255     0.007554976     0.082065261 
% > HPDinterval(MC2.1Sol, 0.95)
%                       lower      upper
% (Intercept)     -0.06409497 0.95629789
% lmunigram       -0.04135399 0.09516229
% lmbigram_online  0.01839468 0.15030825
% lmbigram_retro  -0.06195661 0.06853336
% lmbigram_batch   0.01805916 0.15330165
% attr(,"Probability")
% [1] 0.9498667$
}
