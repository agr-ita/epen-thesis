...
% - [ ] 0) Main
% --- [ ] Alice
% --- [ ] Don't namedrop "LW" and "misperceptions"
...
% - [ ] 3) One/two step
% --- [ ] Schemas
% --- [ ] Add formula Wilson (+ possible proposal for two-step)
...
A few years later, the infant is now an adult with an attuned perceptual system. 
...
%  \begin{overpic}[page=3, width=0.7\linewidth]{chapter02/ahpa_tiles}\end{overpic}
  \caption{{\color{red}[TODO]: \textit{Schematisation of two-step (left) and one-step (right) proposals.}}}
  \label{fig:ahpa_spk}
\end{figure}
...
\cite{wilson2013} qualifies the process as a process of reverse inference within a Bayesian framework, where the perceptual system computes $P([x_i]  | \{z\})$ the posterior probability of candidate percepts $[x_i]$ given the auditory input $\{z\}$. These are estimated, for each candidate percept, from  the product of $P(\{z\}|[x_{i}])$ the likelihood of the acoustics given the percept and $P([x_{i}])$ the prior probability of the percept, defined as its phonotactic acceptability. Mathematically, this can be decribed with the formula in equation \ref{eq_onestep}. The final percept then corresponds to the percept with highest posterior probability, namely the percept satisfying $\underset{x}{arg\,max} \left \{ P(\{z\}|[x_{i}]) \cdot P([x_{i}]) \right \}$. 
There are two elements on the right side of the equation  

\begin{equation}
  P(\left[x_{i}\right] | \left\{ z\right\} ) \propto P(\left\{ z\right\} |\left[ x_{i} \right]) \cdot P(\left[ x_{i} \right])
  \label{eq_onestep}
\end{equation} 
...
In this thesis I will evaluate computational implementations of one-step theories of nonnative speech perception. While only this restricted family of theories will be evaluated, I would like to encourage further research to be done with similar methodology, whether it is to investigate the same type of theories or not.

Using computational models in order to investigate competing theories is beneficial in several ways. Firstly, the need to translate the theories into model implementations forces model ideators to provide a mathematically and/or algorithmically well defined model, as opposed to more vague and ambiguous verbally defined theories that leave more espace to reader interpretations. Having more rigorous model definitions also allows to better understand competing theories (what is the exact nature of the input? which grammar constraints are applied and how? ...), meaning that it is easier to compare proposals and see where they differ significantly or not. 

Secondly, obtaining a computational implementation of a theory means that it is then possible to derive predictions from the model. It is possible then to qualitatively and quantitatively test these predictions, and compare them to what is observed in behavioural data.
