...
The phenomenon of vowel epenthesis was first studied in the context of loanword adaptations, namely the modification of words from a source language when they are introduced to a borrowing language. Consider the following examples:
\begin{itemize}
...
In this context, vowel epenthesis consists in the insertion of a vowel in the (borrowing) surface form of a word that did not contain said vowel in the underlying representation in the source language.  
...
\cite{wilson2013} qualifies the process as a process of reverse inference within a Bayesian framework, where the perceptual system computes the posterior probability of candidate percepts given the acoustic input. These are estimated, for each candidate percept, from the product of the likelihood of the acoustics given the percept and the prior probability of the percept, defined as its phonotactic acceptability. 
